...
Kinmu-OSは、従業員100名以下の組織を対象にしたセルフホスト型の労務・勤怠管理ソフトです。v0.8は、勤怠・休暇・残業・月次締め・給与連携に、部署別承認経路、承認担当者、横断承認受信箱、差し戻しと不変の改訂履歴、期間限定の承認引継ぎ、管理者代理申請、全申請種別のアプリ内通知を追加します。
...
v0.8の検証範囲、既知の制限、更新時の注意事項は [v0.8リリース候補レビュー](docs/v0.8-release-candidate-review.md) に記録します。シナリオ対応は [v0.8テストシナリオ対応表](docs/v0.8-test-scenario-matrix.md)、前版の記録は [v0.7リリース候補レビュー](docs/v0.7-release-candidate-review.md) に残しています。
