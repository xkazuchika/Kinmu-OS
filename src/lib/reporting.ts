...
export async function managementDashboard(db: AppDatabase, organizationId: string, month: string) {
  ...
  const [[headcount], [openDays], overtime, pendingCorrections, [pendingLeave], projected] =
    await Promise.all([
      db
        .select({ value: count() })
        .from(employees)
        .where(
          and(eq(employees.organizationId, organizationId), ne(employees.status, "terminated")),
        ),
      db
        .select({ value: count() })
        .from(attendanceDays)
