...
export async function GET(request: Request) {
  try {
    ...
    const requestedCaseVersion = url.searchParams.get("caseVersion");
    const caseVersion = requestedCaseVersion ? Number(requestedCaseVersion) : undefined;
    const requestedApprovalType = url.searchParams.get("approvalRequestType");
    const approvalRequestType =
      requestedApprovalType === "attendance_correction" ||
      requestedApprovalType === "leave" ||
      requestedApprovalType === "overtime" ||
      requestedApprovalType === "holiday_work"
        ? requestedApprovalType
        : undefined;
    const requestedProxy = url.searchParams.get("submittedOnBehalf");
    return Response.json({
      logs: await searchAuditLogs(database, {
        ...
        approvalRequestType,
        assignedApproverUserId: url.searchParams.get("assignedApproverUserId") || undefined,
        attendanceRevision:
          attendanceRevision && Number.isInteger(attendanceRevision)
            ? attendanceRevision
            : undefined,
        caseVersion: caseVersion && Number.isInteger(caseVersion) ? caseVersion : undefined,
        ...
        overtimeRequestKind,
        originalApproverUserId: url.searchParams.get("originalApproverUserId") || undefined,
        ...
        submittedOnBehalf:
          requestedProxy === "1" ? true : requestedProxy === "0" ? false : undefined,
        transitionStatus: url.searchParams.get("transitionStatus") || undefined,
        validationResult,
      }),
    });
  } catch (error) {
